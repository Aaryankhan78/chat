  ...
  return (
    <div style={{ backgroundImage: `url(${backgroundImage})` }} className='bg-no-repeat bg-cover'>
          ...
          {allMessage.map((msg, index) => (
            <div key={index} className={`p-1 py-1 rounded w-fit max-w-[280px] md:max-w-sm lg:max-w-md ${user._id === msg?.msgByUserId ? "ml-auto bg-teal-100" : "bg-white"}`}>
              <div className='w-full relative'>
                {msg?.imageUrl && (
                  <img src={msg?.imageUrl} alt="uploaded" className='w-full h-full object-scale-down' />
                )}
                {msg?.videoUrl && (
      ...
      <section className='h-16 bg-white flex items-center px-4'>
        ...
        <div className='relative'>
          <button onClick={() => setShowEmojiPicker(prev => !prev)} className='flex justify-center items-center w-11 h-11 rounded-full hover:bg-primary hover:text-white'>
